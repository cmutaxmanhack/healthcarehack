...
if test_file:
    ...
    for index, row in trait_Df.iterrows():
        if row["efo Traits"] == "antidepressant-induced side effect,response to citalopram,response to buspirone,response to antidepressant":
            ...
            st.write("⚠️ ALERT! You are at risk for General Side Effects: including but not limited to nausea, weight gain, and sleeping trouble.")
            print()
        ...
        if row["efo Traits"] == "antidepressant-induced visual impairment,response to bupropion,response to citalopram,response to antidepressant":
            ...
            st.write("⚠️ ALERT! You are at risk for these side effects: Antidepressant-Induced Visual and Hearing Impairment")
            print()
        ...
        if row["efo Traits"] == "response to citalopram,antidepressant-induced dizziness,response to buspirone,response to antidepressant":
            ...
            st.write("⚠️ ALERT! You are at risk for these side effects: Antidepressant-Induced Dizziness")
            print()
    
        if row["efo Traits"] == "antidepressant-induced sexual dysfunction,response to bupropion,response to antidepressant":
            st.write("Bupropion")
            st.write("⚠️ ALERT! You are at risk for these side effects: Antidepressant-Induced Sexual Dysfunction")
            print()
        
        if row["efo Traits"] == "unipolar depression,response to escitalopram,response to citalopram,mood disorder":
            st.write("⚠️ Unfortunately, based on the genetic information you provided Citalopram and Escitalopram have not proven effective for treating your condition.")
            print()
    
        if row["efo Traits"] == "unipolar depression,response to bupropion,mood disorder":
            st.write("⚠️ Unfortunately, based on the genetic information you provided Bupropion has not proven effective for treating your condition. ")
            print()
    
        if row["efo Traits"] == "response to antidepressant":
            st.write("⚠️ Unfortunately, patients with similar genetic information have exibhited antidepressant treatment resistance.")
